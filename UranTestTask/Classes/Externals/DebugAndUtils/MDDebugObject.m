...
-(instancetype) intitWith:(NSInteger)lineNumber {
    ...
    PRINTFUNCTION(MDCOLOR_NO, "%30s : %-4d ~~ Start             - %s", [self.classCaller UTF8String], self.lineNumber, [self.functionCaller UTF8String]);
...
-(oneway void) release {
    if (self.retainCount == 1) {
        PRINTFUNCTION(MDCOLOR_NO, "%30s : %-4d ~~ Finished in %.3f - %s", [self.classCaller UTF8String], self.lineNumber, CFAbsoluteTimeGetCurrent() - self.startTime, [self.functionCaller UTF8String]);
    }
