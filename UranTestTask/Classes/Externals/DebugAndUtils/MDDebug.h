...
/* BEGIN PRIVATE DEFINES */
    #define MDCOLOR_RED @"\e[fg214,0,0;"
    #define MDCOLOR_YELLOW @"\e[fg208,130,10;"
    #define MDCOLOR_BLUE @"\e[fg0,0,210;"
    #define MDCOLOR_NO @""
...
//Trace function
	 ...
	 */
    #define $c \
             PRINTFUNCTION(MDCOLOR_NO, LOG_FMT "%s ", _FILE, __LINE__, __FUNCTION__);
	 ...
	 */
	#define $return \
                return PRINTFUNCTION(MDCOLOR_NO, LOG_FMT "return %s ", _FILE, __LINE__, __FUNCTION__),
    ...
    __unused static id $__printNSObject(const char *file, const int line, const char *name, id nsObject) {
                PRINTFUNCTION(MDCOLOR_NO, LOG_FMT"%s = %@", file, line, name, nsObject);
                return nsObject;
    }
	 ...
	 */
	#define $w(type, ...) \
                PRINTFUNCTION(MDCOLOR_NO, LOG_FMT"%s = " type, _FILE, __LINE__,  #__VA_ARGS__, (__VA_ARGS__));

	#define	$r(rect) \
                PRINTFUNCTION(MDCOLOR_NO, LOG_FMT"%s = (%f; %f), %f * %f", _FILE, __LINE__, #rect, rect.origin.x, rect.origin.y, rect.size.width, rect.size.height);

	#define	$s(size) \
                PRINTFUNCTION(MDCOLOR_NO, LOG_FMT"%s = %f * %f", _FILE, __LINE__, #size, size.width, size.height);

	#define	$p(point) \
                PRINTFUNCTION(MDCOLOR_NO, LOG_FMT"%s = (%f; %f)", _FILE, __LINE__, #point, point.x, point.y);
	 ...
	 */
	#define $l(format, ...) \
                PRINTFUNCTION(MDCOLOR_NO, LOG_FMT format, LOG_ARGS, ##__VA_ARGS__)
    #define $le(message, ...) \
                PRINTFUNCTION(MDCOLOR_RED, LOG_FMT message, LOG_ARGS, ##__VA_ARGS__)
    #define $lw(message, ...) \
                PRINTFUNCTION(MDCOLOR_YELLOW, LOG_FMT message, LOG_ARGS, ##__VA_ARGS__)
    #define $li(message, ...) \
                PRINTFUNCTION(MDCOLOR_BLUE, LOG_FMT message, LOG_ARGS, ##__VA_ARGS__)
	 ...
	 */
	#define $if(...) \
                PRINTFUNCTION(MDCOLOR_NO, LOG_FMT"if (%s) == %s", _FILE, __LINE__, #__VA_ARGS__, ((__VA_ARGS__) ? "true" : "false")); \
                if (__VA_ARGS__)			
...
#define $epicfail(condition, format, ...) \
            if (condition) { \
                PRINTFUNCTION(MDCOLOR_NO, LOG_FMT"fail: " format "%@", _FILE, __LINE__, ##__VA_ARGS__, [NSThread callStackSymbols]); \
                abort(); \
            }
#endif
